...
let movies = [
{
  ...
  "Runtime": "162 min",
  "Genre": "Adventure", //Action, Adventure, Sci-Fi
...
//Movies: Find Title in json
app.get('/movies/:title', (req,res) => {
  const movie = movies.find(s => s.Title === req.params.title);
  if (!movie) { res.status(404).send('given id was not found')
  } else { 
  res.send(movie); 
  };
});
...
//READ 
//Movies: Find Genre in json
app.get('/movies/genre/:genreName', (req,res) => {
  const {genreName} = req.params; 
  const genre = movies.find(movie => movie.Genre === genreName).Title;
  if (genre) { 
...
app.put('/users/:id',(req,res) => {
  ...
  // dont use strict quality because req params is a string
...
app.delete('/users/:id/:movieTitle',(req,res) => {
  ...
  if (user) {
    ...
    res.status(200).send(`${movieTitle} has been deleted from users ${id}s`);
  } else {
    res.status(400).send("no such user") 
  }
})
...
app.delete('/users/:id',(req,res) => {
  ...
  if (user) {
    ...
    res.json(users)
    //res.status(200).send(`${id} has been deleted from userss`);
  } else {
    res.status(400).send("no such user") 
  }
})
